    ...
    <div className="min-h-screen bg-[#F8F9FA] font-sans text-[#0F172A] flex flex-col antialiased">
          ...
          <div className="w-full lg:w-1/3">
            ...
            <div className="flex gap-2">
              <div className="flex-grow flex items-center bg-white border border-[#0F172A]/8 rounded-xl px-4 py-3 shadow-[0px_4px_24px_0px_rgba(15,23,42,0.02)]">
                ...
                <span className="text-xs sm:text-sm text-[#0F172A] font-semibold">Showing {filteredCars.length} available vehicles</span>
              </div>
            </div>
          </div>
      ...
      <section className="py-12 sm:py-16 px-4 sm:px-6 lg:px-8 max-w-[1200px] mx-auto w-full flex-grow">
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredCars.map((car) => (
              <article key={car.id} className="border border-[#0F172A]/8 rounded-2xl overflow-hidden flex flex-col bg-white shadow-[0px_4px_24px_0px_rgba(15,23,42,0.04)] hover:shadow-[0px_4px_24px_0px_rgba(15,23,42,0.08)] transition-all duration-300 hover:-translate-y-0.5">
                ...
                <div className="p-6 flex-grow flex flex-col">
                  <div className="flex justify-between items-start mb-2">
                    <h3 className="text-lg sm:text-xl font-semibold text-[#0F172A] line-clamp-1">{car.title}</h3>
                  </div>
                  
                  <div className="mb-3 space-y-0.5">
                    {car.sales && car.price > 0 && (
                      <p className="font-bold text-[#0F172A] text-base sm:text-lg">
                        ${formatUSD(car.price)} <span className="text-xs font-normal text-[#64748B]">Purchase</span>
                      </p>
                    )}
                    {car.rent && car.rentPrice && (
                      <p className="font-bold text-[#0B57FF] text-base sm:text-lg">
                        ${formatUSD(car.rentPrice)}/day <span className="text-xs font-normal text-[#64748B]">Rental Rate</span>
                      </p>
                    )}
                  </div>
                  ...
                  <div className="grid grid-cols-3 gap-2 mb-5 text-xs text-[#64748B] bg-[#F8F9FA] p-3 rounded-xl border border-[#0F172A]/8">
                    <div className="flex items-center gap-1">
                      <Users className="w-3.5 h-3.5 text-[#0B57FF]" />
                      <span className="truncate">{car.seats} Seats</span>
                    </div>
                    <div className="flex items-center gap-1">
                      <Gauge className="w-3.5 h-3.5 text-[#0B57FF]" />
                      <span className="truncate">{car.transmission}</span>
                    </div>
                    <div className="flex items-center gap-1">
                      <Fuel className="w-3.5 h-3.5 text-[#0B57FF]" />
        ...
        {activeCar && (
          <div className="mt-14 pt-10 border-t border-[#0F172A]/8">
            ...
            <GoogleLocationMap
              ...
              category="car"
              priceDisplay={activeCar.rent && activeCar.rentPrice ? `$${formatUSD(activeCar.rentPrice)}/day Rental` : (activeCar.sales && activeCar.price > 0 ? `$${formatUSD(activeCar.price)} Sale` : undefined)}
            />
          </div>
        )}
      </section>
      ...
      <section className="bg-white py-16 md:py-[113px] border-t border-b border-[#0F172A]/8">
        <div className="max-w-[1200px] mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center max-w-2xl mx-auto mb-12">
            ...
            <h2 className="text-2xl sm:text-3xl font-medium tracking-[-0.03em] text-[#0F172A]">VIP Mobility &amp; Fleet Standards</h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-center">
            <div className="flex flex-col items-center p-8 md:p-10 rounded-2xl bg-[#F8F9FA] border border-[#0F172A]/8 shadow-[0px_4px_24px_0px_rgba(15,23,42,0.02)]">
              ...
              <h3 className="text-lg sm:text-xl font-semibold mb-2 text-[#0F172A]">Diplomatic &amp; Executive Security</h3>
            ...
            <div className="flex flex-col items-center p-8 md:p-10 rounded-2xl bg-[#F8F9FA] border border-[#0F172A]/8 shadow-[0px_4px_24px_0px_rgba(15,23,42,0.02)]">
              ...
              <h3 className="text-lg sm:text-xl font-semibold mb-2 text-[#0F172A]">Pristine Fleet Condition</h3>
              <p className="text-xs sm:text-sm text-[#64748B] leading-relaxed">
            ...
            <div className="flex flex-col items-center p-8 md:p-10 rounded-2xl bg-[#F8F9FA] border border-[#0F172A]/8 shadow-[0px_4px_24px_0px_rgba(15,23,42,0.02)]">
              ...
              <h3 className="text-lg sm:text-xl font-semibold mb-2 text-[#0F172A]">Flexible Purchase &amp; Rental</h3>
              <p className="text-xs sm:text-sm text-[#64748B] leading-relaxed">
      ...
      <section className="bg-white py-16 md:py-20 text-center border-t border-[#0F172A]/8">
        <div className="max-w-4xl mx-auto px-4 sm:px-6">
          <h2 className="text-3xl sm:text-4xl font-medium tracking-[-0.03em] mb-3 text-[#0F172A]">Ready to take the wheel?</h2>
...
export default function CarsPage() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-[#F8F9FA] flex items-center justify-center text-[#64748B]">Loading vehicles...</div>}>
      <CarsContent />
    </Suspense>
  );
}
